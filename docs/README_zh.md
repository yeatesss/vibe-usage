...
- **菜单栏面板** — 一眼看到今天 / 本周 / 本月 / 今年的用量合计；支持「费用 ⇄ Token」切换的 hero 卡片，并附带迷你折线图。
- **完整统计窗口** — 按小时/按天的堆叠柱状图、token 分类（输入、输出、缓存读、缓存写）、单请求平均值、费用趋势，以及 GitHub 风格的 **52 周活跃度热力图**。
- **按项目维度** — 顶部 概览 / 项目 两个视图：概览页用项目筹码条带一键过滤数据；项目页采用 master/detail 钻取单个 repo，支持搜索、按花费/Token/请求数/最近活跃/名称排序，展示分支拆解，并可一键复制项目路径。
- **双工具支持** — 在 Claude Code 与 Codex 之间切换；各自的定价独立展示（Claude `$3 / $15 per 1M`，缓存 90% 折扣；Codex `$1.25 / $10 per 1M`，缓存输入 `$0.125`），费用自动计算。
- **增量采集** — Go 后端监听日志文件，每个 tick 只处理增量；采集间隔可在 30s / 1m / 5m 之间切换。
- **双语界面** — 中文 / English，可在偏好设置中切换。
- **菜单栏自定义** — 仅图标、显示今日花费、或显示 token 总量；主题可选 跟随系统 / 浅色 / 深色；支持开机启动。
- **键盘友好** — `⌘1` 概览、`⌘2` 项目、`Esc` 清除筛选、`⌘⇧U` 打开仪表盘、`⌘,` 偏好设置、`⌘Q` 退出。
...
| 菜单栏 — Claude Code | 菜单栏 — Codex |
|---|---|
| <img src="img/tooltip_claude_zh.png" width="280" /> | <img src="img/tooltip_codex_zh.png" width="280" /> |

完整仪表盘：

<img src="img/dashboard_zh.png" width="780" />
...
- **前端**（`Sources/Usage/`，Swift 6.2 / SwiftUI，macOS 26+）：菜单栏控制器、popover、完整统计窗口、项目 master/detail、偏好设置窗口、Swift Charts 渲染、活跃度热力图。
- **后端**（`backend/`，Go 1.25，Gin + modernc SQLite）：跟踪会话日志、将不同工具的事件结构归一化、调用 `pricing/` 计算费用，提供 `/usage`、`/usage/heatmap`、`/usage/projects`、`/health`、`/version`、`/config/tick` 接口。
...
大部分设置都集中在 **偏好设置**（`⌘,`）里：语言、刷新间隔（30s / 1m / 5m）、开机启动、菜单栏显示形式、主题、以及数据源连接状态。
